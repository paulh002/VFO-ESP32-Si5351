...
Si5351::Si5351(uint8_t i2c_addr, uint8_t i2c_sda, uint8_t i2c_scl, uint8_t _bus)
{
...
bool Si5351::init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
	...
	if (g_bus != bus)
	{
		if (bus < 8)
		{
			if (!i2c.i2c_send_byte(TCA9548A_ADDR, 1 << bus))
				return 1;
			g_bus = bus;
		}
	}
